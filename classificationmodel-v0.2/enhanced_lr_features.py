...
class EnhancedFeatureExtractor:
        ...
        for text in texts:
            ...
            try:
                if TEXTSTAT_AVAILABLE:
                    # Use getattr to safely access textstat functions
                    flesch_func = getattr(textstat, 'flesch_reading_ease', None)
                    syllable_func = getattr(textstat, 'syllable_count', None)
                    
                    if flesch_func and syllable_func and text_str.strip():
                        readability = flesch_func(text_str)
                        syllables = syllable_func(text_str)
    ...
    def extract_all_features(self, texts, is_training=True) -> Any:
        ...
        try:
            if all_features is not None and hasattr(all_features, 'shape'):
                # Type assertion to help Pylance understand all_features is not None
                num_features = all_features.shape[1]  # type: ignore
                print(f"✅ Extracted {num_features} enhanced features")
                ...
                print("❌ Error: Failed to combine features")
...
def enhanced_logistic_regression_pipeline(train_data, test_data, target_column='Category L2'):
    ...
    X_train_raw = feature_extractor.extract_all_features(train_data['Item_Descripton'], is_training=True)
    X_test_raw = feature_extractor.extract_all_features(test_data['Item_Descripton'], is_training=False)
    
    # Ensure sparse matrices are in CSR format for sklearn compatibility
    X_train: Any = X_train_raw
    X_test: Any = X_test_raw
    if hasattr(X_train_raw, 'tocsr'):
        X_train = X_train_raw.tocsr()
    if hasattr(X_test_raw, 'tocsr'):
        X_test = X_test_raw.tocsr()
