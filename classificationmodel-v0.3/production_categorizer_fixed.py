...
class ProductionCategorizer:
    ...
    def _load_production_config(self, config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    def prepare_training_data(self, df: pd.DataFrame, 
                            ...
                            category_col: str = 'Category L2') -> Tuple[pd.Series, pd.Series]:
...
def main_production_demo():
    ...
    for idx, (desc_idx, desc) in enumerate(sample_descriptions.items()):
        pred_row = predictions.loc[desc_idx]
        conf = pred_row.get('confidence', 0.0)
        print(f"   {idx+1}. '{str(desc)[:50]}...' → {pred_row['predicted_category']} ({conf:.2f})")
    ...
    if info['training_info']:
        print(f"   Validation Accuracy: {info['training_info'].get('val_accuracy', 0):.2f}%")
    print(f"   Model File: {model_path}")
    
    # Recommendations
    if info['training_info']:
        val_accuracy = info['training_info'].get('val_accuracy', 0)
