...
class EnhancedSVMTrainer:
    ...
    def detailed_evaluation(self, model_results, model_name):
        ...
        # Use the appropriate label encoder for this model
        if 'label_encoder' in model_results:
            current_encoder = model_results['label_encoder']
        else:
        ...
        for cat in target_names:
            if cat in report:
                f1_score = report[cat].get('f1-score', 0)
                category_f1.append((cat, f1_score))
        ...
        for idx in sample_indices:
            ...
            pred_cat = current_encoder.inverse_transform([y_pred[idx]])[0]
